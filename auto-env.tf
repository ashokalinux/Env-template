...
resource "local_file" "gp_auto_environment_file" {
  content = templatefile("${path.root}/Auto/Lab.auto_environment.html.tpl", {
    ...
    GITLAB_SERVER_PUB                  = (data.terraform_remote_state.boot.outputs.gitlab_server_ip),
    GITLAB_CI_PRI                      = (data.terraform_remote_state.boot.outputs.gitlab_ci_private_ip),
    GITLAB_CI_PUB                      = (data.terraform_remote_state.boot.outputs.gitlab_ci_public_ip),
